...
Ext.define('Ext.ux.Router', {
     ...
     */
    init : function(app) {
        var me = this, history = Ext.History;
    ...
    routeMatcher : function(app, route, rules) {
        var routeObj, action, me = this, routes = me.routes, reRoute = route, reParam = /([:*])(\w+)/g, reEscape = /([-.+?\^${}()|\[\]\/\\])/g, names = [];
    ...
    parse : function(token) {
        var route, matches, params, names, j, param, value, rules, tokenArgs, tokenWithoutArgs, me = this, routes = me.routes, i = 0, len = routes.length;
    ...
    getController : function(route) {
        var controllerFullName, controllerCapitalized, app = route.app, classMgr = Ext.ClassManager, controller = route.controller;
